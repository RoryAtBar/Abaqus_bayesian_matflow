...
if kern == "RBF":
    ...
    (X_train, Y_train),
    kernel=gpflow.kernels.RBF(np.shape(X_train)[-1], lengthscales=np.ones(np.shape(X_train)[-1]))+
    ...
    model = gpflow.models.GPR(
    (X_train, Y_train),
    kernel=gpflow.kernels.Matern52(np.shape(X_train)[-1], lengthscales=np.ones(np.shape(X_train)[-1]))+
    gpflow.kernels.Linear(np.shape(X_train)[-1]),
    )
    print(model)    
elif kern == "Matern32":
    model = gpflow.models.GPR(
    (X_train, Y_train),
    kernel=gpflow.kernels.Matern32(np.shape(X_train)[-1], lengthscales=np.ones(np.shape(X_train)[-1]))+
    gpflow.kernels.Linear(np.shape(X_train)[-1]),
    )
    print(model)  
elif kern == "Matern12":
    ...
    score = 1e20
    kernels = [[gpflow.kernels.RBF(np.shape(X_train)[-1], lengthscales=np.ones(np.shape(X_train)[-1]))+
        gpflow.kernels.Linear(np.shape(X_train)[-1])],
               [gpflow.kernels.Matern52(np.shape(X_train)[-1], lengthscales=np.ones(np.shape(X_train)[-1]))+
        gpflow.kernels.Linear(np.shape(X_train)[-1])],
               [gpflow.kernels.Matern32(np.shape(X_train)[-1], lengthscales=np.ones(np.shape(X_train)[-1]))+
        gpflow.kernels.Linear(np.shape(X_train)[-1])],
               [gpflow.kernels.Matern12(np.shape(X_train)[-1], lengthscales=np.ones(np.shape(X_train)[-1]))+
        gpflow.kernels.Linear(np.shape(X_train)[-1])]]
